...
class APIClient:
    ...
    def predict(self, payload: dict) -> dict:
        ...
        """
        response = requests.post(config.API_PREDICT_ENDPOINT, json=payload, timeout=self.timeout)
    ...
    def get_accidents(self, offset: int = 0, limit: int = 100) -> list:
        ...
        response = requests.get(config.API_ACCIDENTS_ENDPOINT, params=params, timeout=self.timeout)
        response.raise_for_status()
        return response.json()
