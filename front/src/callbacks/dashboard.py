...
def register_dashboard_callbacks(app):
    ...
    def update_dashboard(dep, saison, gravite):
        ...
        fig_gravite = px.histogram(
            dff, x="gravite", color="gravite",
            title="Distribution de la gravité"
        )
        
        fig_saison = px.histogram(
            dff, x="saison", color="gravite",
            title="Gravité par saison"
        )
        
        fig_dep = px.histogram(
            dff, x="dep", color="gravite",
            title="Gravité par département"
        )
        
        fig_lum = px.histogram(
            dff, x="lum", color="gravite",
            title="Gravité par luminosité"
        )
        
        fig_atm = px.histogram(
            dff, x="atm", color="gravite",
            title="Gravité par météo"
        )
        
        fig_nbveh = px.scatter(
            dff, x="nb_vehicules", y="nb_usagers",
            color="gravite",
